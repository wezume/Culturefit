...
const CultureFitScorer = () => {
  ...
  const [activeTab, setActiveTab] = useState('design'); // 'design' or 'analysis'
    ...
    const fetchCandidates = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Map backend scores to what frontend expects for radar chart
          const formatted = data.map(c => ({
            ...c,
            fitScore: c.cultureFitScore || 0,
            isAccepted: (c.cultureFitScore || 0) >= 70,
            inputValues: { // Placeholder for signals if not already present
              emotion: 0.8, smile: 0.7, eyeContact: 0.9, tone: 0.8, pitch: 0.7, 
              energy: 0.8, speechRate: 0.5, straightFace: 0.2, fillerWords: 0.1
            }
          }));
  ...
  return (
    <div className="min-h-screen w-full bg-[#fcfdfe] text-slate-800 font-sans flex flex-col selection:bg-indigo-100 selection:text-indigo-900">
      {/* Navigation Header */}
      <header className="sticky top-0 z-[100] w-full bg-white/70 backdrop-blur-2xl border-b border-slate-100 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-black text-slate-900 tracking-tight leading-none">CultureFit</h1>
              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">HR Intelligence Hub</p>
            </div>
          </div>
          ...
          <div className="flex items-center bg-slate-100/50 p-1.5 rounded-2xl border border-slate-200/50">
            <button
              ...
              className={`flex items-center gap-2.5 px-6 py-2.5 rounded-xl text-[12px] font-black uppercase tracking-widest transition-all duration-300 ${activeTab === 'design'
                ? 'bg-white text-indigo-600 shadow-md shadow-slate-200 border border-slate-100'
                : 'text-slate-400 hover:text-slate-600'
                }`}
            >
              <Target size={16} /> 1. Culture Map
            </button>
            <button
              onClick={() => setActiveTab('analysis')}
              className={`flex items-center gap-2.5 px-6 py-2.5 rounded-xl text-[12px] font-black uppercase tracking-widest transition-all duration-300 ${activeTab === 'analysis'
                ? 'bg-white text-indigo-600 shadow-md shadow-slate-200 border border-slate-100'
                : 'text-slate-400 hover:text-slate-600'
                }`}
            >
              <Users size={16} /> 2. Analysis
            </button>
          </div>
        </div>
      </header>
      ...
      <main className={`flex-1 ${!selectedIndustry ? 'max-w-7xl' : 'max-w-full lg:px-12'} mx-auto w-full p-4 sm:p-6 lg:p-8 flex flex-col gap-6 sm:gap-8`}>
        ...
        {selectedIndustry && activeTab === 'design' && (
              ...
              <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-xl shadow-slate-200/50 border border-slate-100 p-6 sm:p-7 sticky top-24">
                ...
                <div className="space-y-6">
                  ...
                  <button
                    onClick={async () => {
                      if (!isAccepted) {
                        const confirmLock = window.confirm("Are you sure you want to lock this fit profile? It will be locked for 3 months and cannot be modified.");
                        if (!confirmLock) return;

                        setIsAccepted(true);

                        try {
                          await fetch('http://localhost:8000/api/recruiter/lock', {
                            method: 'POST',
                            headers: { 'Content-Type': 'application/json', 'x-auth-token': localStorage.getItem('token') || '' },
                            body: JSON.stringify({ role: selectedRole || 'default' })
                          });
                        } catch (err) {
                    ...
                    className={`w-full mt-10 py-4 rounded-2xl font-black text-[13px] uppercase tracking-widest transition-all flex items-center justify-center gap-3 shadow-xl ${
                      isAccepted 
                        ? 'bg-slate-50 text-slate-400 border border-dashed border-slate-200 shadow-none' 
                        : 'bg-indigo-600 text-white hover:bg-indigo-700 shadow-indigo-100'
                    }`}
                  >
                    {isAccepted ? 'reset n modify' : (
        ...
        {/* Analysis Phase */}
        {selectedIndustry && activeTab === 'analysis' && (
          <div className="flex flex-col gap-8 max-w-5xl mx-auto w-full animate-in fade-in slide-in-from-bottom-8 duration-700">
            <div className="bg-white rounded-3xl p-8 border border-slate-100 shadow-xl shadow-slate-200/50">
              <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
                <div>
                  <h2 className="text-2xl font-black text-slate-900 tracking-tight">Candidate Fit Analysis</h2>
                  <p className="text-slate-500 font-medium">Select a candidate to view their cultural alignment with your map.</p>
                </div>
                
                <div className="relative w-full md:w-80">
                  <button
                    onClick={() => setIsRoleDropdownOpen(!isRoleDropdownOpen)}
                    className="w-full px-5 py-3.5 bg-slate-50 border border-slate-100 rounded-2xl text-[13px] font-bold text-slate-800 flex items-center justify-between hover:border-indigo-300 transition-all shadow-sm"
                  >
                    <div className="flex items-center gap-3">
                      <Search size={18} className="text-slate-400" />
                      <span>{selectedCandidateId ? candidateData.find(c => c.id === selectedCandidateId)?.name : 'Select Candidate...'}</span>
                    </div>
                    <ChevronDown size={18} className={`text-slate-400 transition-transform ${isRoleDropdownOpen ? 'rotate-180' : ''}`} />
                  </button>
                  {isRoleDropdownOpen && (
                    <div className="absolute top-full left-0 right-0 mt-2 bg-white border border-slate-100 rounded-2xl shadow-2xl z-[60] overflow-hidden py-2 max-h-60 overflow-y-auto">
                      {candidateData.map(c => (
                        <button
                          key={c.id}
                          onClick={() => {
                          ...
                          className={`w-full text-left px-5 py-3 text-sm font-bold transition-colors ${selectedCandidateId === c.id ? 'bg-indigo-50 text-indigo-600' : 'text-slate-600 hover:bg-slate-50'}`}
                        >
                          {c.name} ({c.fitScore.toFixed(0)}%)
                        </button>
                      ))}
                    </div>
                  )}
              ...
              {!selectedCandidateId ? (
                <div className="py-24 text-center border-2 border-dashed border-slate-100 rounded-3xl">
                  ...
                  <p className="text-slate-400 font-bold uppercase tracking-widest">Select a candidate to begin analysis</p>
                </div>
              ) : (
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                  <div className="flex justify-center p-8 bg-slate-50/50 rounded-3xl border border-slate-50 relative">
                    <div className="w-full max-w-[320px] aspect-square">
                      <RadarChartCustom
                        targetScores={currentTargets}
                        candidateScores={candidateData.find(c => c.id === selectedCandidateId)?.culturalScores || [0,0,0,0,0]}
                        labels={['Teamwork', 'Excellence', 'Integrity', 'Innovation', 'Quality']}
                      />
                    </div>
                  </div>
                  ...
                  <div className="space-y-8">
                    <div className="flex items-center gap-4 bg-indigo-50/50 p-6 rounded-3xl border border-indigo-100/50">
                      <div className="p-4 bg-white rounded-2xl shadow-sm border border-indigo-50">
                        <Award size={32} className="text-indigo-600" />
                      </div>
                      <div>
                        <p className="text-[10px] font-black text-indigo-400 uppercase tracking-[0.2em] mb-1">Overall Fit Score</p>
                        <h4 className="text-4xl font-black text-slate-900 tracking-tighter">
                          {candidateData.find(c => c.id === selectedCandidateId)?.fitScore.toFixed(0)}%
                        </h4>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      {['Teamwork', 'Excellence', 'Integrity', 'Innovation', 'Quality'].map((trait, i) => {
                        const score = candidateData.find(c => c.id === selectedCandidateId)?.culturalScores?.[i] || 0;
                        const target = currentTargets[i];
                        const diff = score - target;
                        return (
                          <div key={trait} className="p-5 bg-white rounded-2xl border border-slate-100 shadow-sm">
                            <div className="flex justify-between items-center mb-2">
                              <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest">{trait}</p>
                              <span className={`text-[10px] font-black px-2 py-0.5 rounded-lg ${diff >= 0 ? 'bg-emerald-100 text-emerald-600' : 'bg-amber-100 text-amber-600'}`}>
                                {diff >= 0 ? 'Exceeds' : 'Gap'}
                              </span>
                            </div>
                            <div className="flex items-end gap-2 text-slate-900">
                              <span className="text-xl font-black">{score.toFixed(1)}</span>
                              <span className="text-[10px] font-bold text-slate-400 mb-1">/ target {target}</span>
                            </div>
                          </div>
                        );
                      })}
